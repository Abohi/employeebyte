...
class AddEmployeePage extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final designation = useState<String?>("");
                                  ...
                                  children: <Widget>[
                                    Theme(
                                      ...
                                      child: TextFormField(
                                        ...
                                        onSaved: (value)=>designation.value=value,
                                        style: GoogleFonts.roboto(textStyle: TextStyle(color: const Color(0xff29283C),fontSize: 14)),
                                        decoration: InputDecoration(
                                          filled: true,
                                          fillColor: Colors.white,
                                ...
                                CustomButton(buttonBg: colorPrimary, buttonTitle: "CONTINUE", buttonFontColor: colorWhite, onButtonPressed: () async{
                                  if (_formKey.currentState!.validate()) {
                                    ...
                                    if(selectedCountry.value!=null && selectedState.value!=null && dateOfBirth.value!=null && selectedSex.value!=null && photo.value!=null){
                                      ...
                                      progress!.showWithText('Registering Employee...');
                                      await  context.read(employeeControllerProvider.notifier).saveEmployeeRecord({
                                      ...
                                      var contains = context.read(employeeControllerProvider)!.where((element) => (element.firstName == context.read(employeeStateProvider).state!.firstName) && (element.lastName== context.read(employeeStateProvider).state!.lastName));
                                      if(!contains.isEmpty){
                                        await Fluttertoast.showToast(msg: "Employee details saved successfully",toastLength: Toast.LENGTH_LONG);
                                      }else{
                                        await Fluttertoast.showToast(msg: "Employee details could not be saved",toastLength: Toast.LENGTH_LONG);
                                      }
                                      progress.dismiss();
                                    }else{
